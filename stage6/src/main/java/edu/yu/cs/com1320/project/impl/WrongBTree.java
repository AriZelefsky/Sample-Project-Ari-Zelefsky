 ...
 */
public class WrongBTree<Key extends Comparable<Key>, Value> //in my version of the class, impl class, it will say the same thing,
    //but the singular time i actually instantiate it, in docstore contructor,
    //plug in URI (which does extend comparable) as key and DOcument as Value
{
    ...
    private static final class Node {
        ...
        private Node next; //az not needed
        private Node previous; //az not needed
        ...
        private Entry[] getEntries() //az not sure if needed
        {
    ...
    private static class Entry {
        private Comparable key; //az iT this will become URI
        private Object val;
        private Node child;
     ...
     * @return
     */
    //AZ potentially not relevant to project
    ...
    //Probably useless for project. who cares what smallest URI is.
    public Entry getMinEntry() {
        Node current = this.leftMostExternalNode;
        while (current != null) {
            for (Entry e : current.getEntries()) {
                if (e.val != null) //az this, which was laready here, ensures we do not include sentinel entry
                {
                    return e;
                }
            }
    ...
    //Probably useless for project. who cares what Biggest URI is.
    ...
    public Value get(Key key) {
        if (key == null) {
            throw new IllegalArgumentException("argument to get() is null"); //AZ be careful. THis is copied from wrongBTree. Is this proper
        }
    ...
    private Entry get(Node currentNode, Key key, int height) {
        ...
        if (height == 0) {
            for (int j = 0; j < currentNode.entryCount; j++) //entry count not being max, but rather actual amount of non-nulls in this node
            {
    ...
    public void put(Key key, Value val) {
        ...
        if (alreadyThere != null) {
            alreadyThere.val = val;  //az urgent does not delete if val is null. leaves it there with a null val
            //az yet a non-null key with a null value is supposed to indicate something that has been written to disk! is that always happening every time there is a deletion or will deletion sometimes stay local if sufficient memory
            return;
        }

        Node newNode = this.put(this.root, key, val, this.height);
        this.n++;
        if (newNode == null) //az iT that split did not occur, so having recused back to this step, this height of top level of tree, there is no new node to squeeze in next to the root
        {
            return;
        }
    ...
    private Node put(Node currentNode, Key key, Value val, int height) {
        ...
        if (height == 0) {
            ...
            for (j = 0; j < currentNode.entryCount; j++) {
                if (less(key, currentNode.entries[j].key)) //az if less than this very entry, we will put new entry right here, and slide away this very entry to the right
                {
                    break;
                }
            }
        ...
        //AZ for both leaf or internal node:
    ...
    // comparison functions - make Comparable instead of Key to avoid casts  az: not sure if this will hold true given that i am using <key extends comparable<key>>
