...
public class TrieImpl<Value> implements Trie<Value> {
     ...
     */
    private class Node{
        //Map of every Value (Document) that has this node's word in it, and how many times it does
     ...
     * @param val AZ: Value will represent a Document, which will be added to a List<Value> of Documents in a node field.
    ...
    public void put(String key, Value val){
        if (val == null) this.deleteAll(key); //null value is a delete. Delete ALL VALUES (all docs) previously associated with the trieNode at the end of this keyword String.
    ...
    private Node put(Node x, String key, Value val, int d){
        ...
        //when final node is reached. Ready to put Value (Document) into node's wordHits map
        if(d == key.length()){
            //If this value (Document) already exists at this key (at the node at the end of this string)
            //then update the counter of how many times this specific word appears in this specific document
    ...
    public List<Value> getSorted(String key, Comparator<Value> comparator){
        ...
        sortedValueList.addAll(x.wordHits.keySet()); //Add all wordHits (Documents), which are stored in the KEYS of a hashmap called wordHits (this is due to Prof referring to the type of these wordHits as being <Value>, but needing them to be keys to maintain lack of duplicates
        sortedValueList.sort(comparator); //The comparator will, for each document in this item, of type Value (Documents) in the list, look up that item in the Node.wordHits map, glance at its integer value that represents word count of this word in this doc, and sort accordingly.
    ...
    public Set<Value> get(String key){
        ...
        if(key==null || key.equals("")) return valueSet; //Returns empty set (there are no matches, so specs demand empty set return)
        Node x = this.getNode(this.root, key, 0);

        if(x == null) return valueSet; //returns empty set

        valueSet.addAll(x.wordHits.keySet()); //Add all Values (Documents), which are keys in Node.value, associated with this key (this word) (Confusing use of key and wordHits to maintain required method headers)
        return valueSet;
    }
